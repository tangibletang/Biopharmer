...
def biologist_agent(state: WarRoomState) -> dict:
    ...
    response = _llm.invoke([
        ...
        HumanMessage(content=(
            f"Ticker: {ticker}\n\n"
            f"Mechanism:\n{mechanism}\n\n"
            f"Clinical snapshot: Emax={clinical.get('emax_pct')}% of normal dystrophin, "
            ...
            "Defend the efficacy case for this mechanism."
        )),
    ])
...
def synthesizer_agent(state: WarRoomState) -> dict:
    ...
    response = _llm.invoke([
        SystemMessage(content=(
            ...
            "a concise JSON object. The actionable_metric must be a specific, measurable "
            "data point investors should monitor (e.g., a trial readout date, a dystrophin "
            "threshold, a safety trigger). Respond ONLY with a valid JSON object — no markdown, "
